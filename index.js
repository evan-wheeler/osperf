...
if (0) {
    var schema = JSON.parse(
        fs.readFileSync("./coldump.1.json", { encoding: "utf-8" })
    );
    var schema2 = JSON.parse(
        fs.readFileSync("./coldump.json", { encoding: "utf-8" })
    );
    ...
    for (s of schema) {
        let ltbl = s.tbl.toLowerCase();
        let lcol = s.col.toLowerCase();

        // case insensitive looking on tables...
        let tbl = tables[ltbl];
        if (!tbl) {
            tbl = tables[ltbl] = {};
            tbl.name = s.tbl;
        ...
        // case-insensitive lookup on column name
        let col = cols[lcol];
