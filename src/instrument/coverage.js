...
module.exports = function coverage(src, parseNode, idGenerator, options) {
  ...
  var ctx = new BlockTracker({
    ...
    headerFn: function (block, insertPos) {
      editList.insert(
        "Assoc __cov = Assoc.CreateAssoc( 0 )\n",
        insertPos,
        "after"
      );
  ...
  var forLoop = function (node, body) {
    ...
    ctx.startBlock(
      Block.TYPES.LOOP,
      this.getStartLine(body[0]),
      this.getStartPos(body[0])
    );
  };
  ...
  walker
    .on({
      ...
      "before:ElseifStatement.consequent": function (node, consequent) {
        ...
        ctx.addLine(
          this.getStartLine(node.test[0]),
          this.getStartPos(node.test[0])
        );
      },
      ...
      ReturnStatement: function (node) {
        ...
        if (options.timings && funcBlockID) {
          ...
          if (isSimpleOp(arg)) {
            ...
            var expr, start, end;
            expr = src.substring(
              this.getStartPos(arg[0]),
              this.getEndPos(arg[0]) + 1
            );
  ...
  if (ctx.hasRawCode() && !rawScriptFix) {
    ...
    var lastStmt =
      _.isArray(parseNode) && parseNode.length
        ? parseNode[parseNode.length - 1]
        : null;
