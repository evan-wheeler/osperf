var fs = require("fs"),
  path = require("path"),
  zlib = require("zlib");
(Module = require("../module")), (IDGenerator = require("../idgen")), (Q = require("q")), (fs = require("fs")), (async = require("async")), (instrument = require("../instrument/profile")), (_ = require("lodash")), (parseUtils = require("../parseutils"));
  ...
  return parseUtils
    ...
    .then(function() {
      modObjs.forEach(function(m) {
        addHeader(m.getStartupScripts(), gen.getIDs());
      });
...
function processFiles(srcFiles, idGenerator) {
  ...
  return Q.nfcall(
    ...
    srcFiles,
    4,
    processEach.bind(null, idGenerator)
  );
}
...
function writeMapFile(destFile, mapEntries) {
  ...
  zlib.deflate(input, function(err, buffer) {
    if (!err) {
      fs.writeFileSync(destFile, buffer.toString("base64"), {
        encoding: "utf8"
      });
    }
  });
